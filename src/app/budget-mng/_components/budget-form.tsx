...
import { BudgetUpdateToast } from "./budget-update-toast"
...
export const BudgetForm = ({
  ...
  const handleToastOpenChange = React.useCallback((open: boolean) => {
    if (!open) setToastVariant(null)
  }, [])
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await patchMonthlyBudgetDefault({ defaultMonthlyBudget })
      ...
      await postBudgetPeriod({ year, month, budgetAmount })
        ...
        <div className="flex flex-col gap-7">
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center gap-3">
              <span id="monthly-budget-heading" className="text-sm font-medium">
                월별 예산 선택(선택하지 않으면 이번 달 예산 적용)
              </span>
              <PopoverPrimitive.Root open={monthPickerOpen} onOpenChange={setMonthPickerOpen}>
                <PopoverPrimitive.Anchor asChild>
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      value={
                        selectedYearMonth
                          ? formatYearMonth(selectedYearMonth.year, selectedYearMonth.month)
                          : ""
                      }
                      className={cn(
                ...
                <PopoverPrimitive.Portal>
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between gap-2">
                      ...
                      <span className="text-sm font-semibold tabular-nums">{viewYear}년</span>
                      <button
                        type="button"
                        className="inline-flex size-8 items-center justify-center rounded-md hover:bg-accent"
                        onClick={() => setViewYear((y) => y + 1)}
